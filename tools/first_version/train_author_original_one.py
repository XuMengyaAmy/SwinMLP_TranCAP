...
def train(opt):
    ...
    # Build dataloader
    ################################
    loader = DataLoader(opt) #### my: data loader
    ...
    # my: load the infor from last model
    ...
    # ============= my: choose the specific model based on opt.caption_model ======= #
    ...
    lw_model = LossWrapper(model, opt) ################# my: what's the loss it used
    ...
    dp_lw_model = torch.nn.DataParallel(lw_model) # my: the model after DataParallel and LossWrapper
    # # ============ my: multiple gpu ============ #
    ...
    #  Build optimizer
    ##########################
    # my: To figure our which optimizer we are using
    if opt.noamopt:
        # enter this one
        print('--------------- noamopt --------------')
    ...
    # my: need to be further understood
        ...
        while True:
            ...
            if epoch_done:
                if not opt.noamopt and not opt.reduce_on_plateau:
                    print('mengya: will not enter this step 1111111111111')
                ...
                if epoch > opt.scheduled_sampling_start and opt.scheduled_sampling_start >= 0:
                    print('mengya: will not enter this step 22222222222222')
                ...
                if opt.self_critical_after != -1 and epoch >= opt.self_critical_after:
                    print('mengya: will not enter this step 33333333333333333')
                    sc_flag = True
                ...
                if opt.structure_after != -1 and epoch >= opt.structure_after:
                    print('mengya: will not enter this step 44444444444444444')
                    struc_flag = True
                    init_scorer(opt.cached_tokens)
                else:
                    struc_flag = False
            ...
            torch.cuda.synchronize() # my: for testing the running time of the model
            start = time.time()
            ...
            # ---------- send data to mdoel during training ---------#
            ...
            # make evaluation on validation set, and save model (in iterations)
            # if (iteration % opt.save_checkpoint_every == 0 and not opt.save_every_epoch) or \
            #     (epoch_done and opt.save_every_epoch):
            
            # my: make evaluation on validation set, and save model (in epoch)
            if (epoch_done and opt.save_every_epoch):
                # eval model
                # 'dataset' in eval_kwargs will include the keyword "daisi"
                eval_kwargs = {'split': 'val',
                                'dataset': opt.input_json} #################### dataset
...
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! #
                ...
                # my: Therefore, the best model is saved based on the CIDEr metric 
                if opt.language_eval == 1:
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! # 
                    current_score = lang_stats['CIDEr'] # author
                    # current_score = lang_stats['Bleu_1'] # my: debug: try to save the best model based on Bleu_1
                else:
                ...
                if best_flag:
                    best_epoch = epoch
                    print("best epoch: ", epoch, "saved the best checkpoint")
    ...
    print('best_val_CIDEr_score:', best_val_score)
...
# Solution: https://bbs.cvmart.net/articles/5250
