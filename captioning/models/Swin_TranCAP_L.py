...
# class linear512_Vision_Swin_TransformerModel_patchemd_separate_with_attmask_embdim128(AttModel):
class Swin_TranCAP_L(AttModel):
    def make_model(self, opt, src_vocab, tgt_vocab, N_enc=6, N_dec=6, 
        ...
        model = EncoderDecoder(
            SwinTransformer_Encoder(d_model=self.opt.d_model, d_ff=self.opt.d_ff, dropout=self.opt.dropout,
                   img_size=self.opt.image_size, patch_size=self.opt.patch_size, in_chans=self.opt.in_channels,
                   embed_dim=self.opt.dim, 
                   depths=[2, 2, 18, 2 ], num_heads=[4, 8, 16, 32],
                   window_size=self.opt.window_size, qkv_bias=True, qk_scale=None,
                   drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
    ...
    def __init__(self, opt):
        super(Swin_TranCAP_L, self).__init__(opt)
        self.opt = opt
