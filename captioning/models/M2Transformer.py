...
# for dff_512
#  File "/home/ren2/anaconda3/envs/Captioning_Zoo/lib/python3.8/site-packages/m2transformer/models/transformer/transformer.py", line 28, in forward
...
class M2TransformerModel(TransformerModel):
    ...
    # # mengya: 512 set: change d_ff from 2048 into 512
    # def make_model(self, src_vocab, tgt_vocab, N_enc=6, N_dec=6, 
    #            d_model=512, d_ff=512, h=8, dropout=0.1):
    #     "Helper: Construct a model from hyperparameters."
    #     encoder = MemoryAugmentedEncoder(N_enc, 0, d_in=512, d_ff=512, attention_module=ScaledDotProductAttentionMemory,
    #                                     attention_module_kwargs={'m': 40})
    #     # Another implementation is to use MultiLevelEncoder + att_embed
    #     decoder = MeshedDecoder(tgt_vocab, 54, N_dec, -1, d_ff=512) # -1 is padding;
    #     model = Transformer(0, encoder, decoder) # 0 is bos
    #     return model

    # mengya: d_in/att_size = 512, d_ff=2048
    def make_model(self, src_vocab, tgt_vocab, N_enc=6, N_dec=6, 
               d_model=512, d_ff=2048, h=8, dropout=0.1):
        "Helper: Construct a model from hyperparameters."
        encoder = MemoryAugmentedEncoder(N_enc, 0, d_in=512, d_ff=2048, attention_module=ScaledDotProductAttentionMemory,
                                        attention_module_kwargs={'m': 40})
        # Another implementation is to use MultiLevelEncoder + att_embed
        decoder = MeshedDecoder(tgt_vocab, 54, N_dec, -1, d_ff=2048) # -1 is padding;
        model = Transformer(0, encoder, decoder) # 0 is bos
        return model
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        # mengya: the generated att_masks is not used, the encoder of M2Transformer itself will generate att_masks automatically
        # https://github.com/aimagelab/meshed-memory-transformer/blob/master/models/transformer/encoders.py
