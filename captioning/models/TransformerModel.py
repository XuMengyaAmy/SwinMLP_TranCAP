...
class EncoderDecoder(nn.Module):
    ...
    def decode(self, memory, src_mask, tgt, tgt_mask):
        # print('encoder output shape', memory.shape) # encoder output shape torch.Size([3, 196, 512])
...
class Encoder(nn.Module):
    ...
    # def flops(self):
    #     flops = 0
    #     flops += 4 * vocab *d_model +vocab + 6 * (16* vocab *d_model + h * (6*d_model*vocab*d_k - 4*vocab*d_k + 4*d_k*vocab*vocab + 3*vocab*vocab - vocab) +2 * vocab * d_model *d_model - vocab * d_model + 2 * vocab + 4 * vocab * d_model *d_ff)
...
class Decoder(nn.Module):
    ...
    # def flops(self):
    #     flops = d_out * ( 4 * self.d_model +1) + 6 * (16 * d_out *d_model + 2 * d_out * d_model * d_model + 4*d_model*d_out*d_ff + 4 * d_model * vocab + vocab +2 * d_model * d_model * d_out + h * (8 * d_model * d_out * d_k - 6 * d_k + 4 * d_k * d_out * d_out + 3 * d_out * d_out - 2 * d_out +4 * d_model * vocab * d_k - 2 * d_k * vocab + 4 * d_k * d_out * vocab + 3 * vocab * d_out))
    #     return flops
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, memory, src_mask, tgt_mask):
        ...
        # print('========= start =========')
        # print('mengya: cross attention in decoder')
...
class Embeddings(nn.Module):
    ...
    # def flops(self):
    #     # Input_embeding的FLOPs = vocab*d_model*2
    #     flops = self.vocab*self.d_model*2
    #     return flops
...
class TransformerModel(AttModel):
    ...
    def __init__(self, opt):
        ...
        print('&'*30)
        print('tgt_vocab', tgt_vocab) # vocab size is  36 + 1 = 37
        print('&'*30)
    ...
    def _prepare_feature_forward(self, att_feats, att_masks=None, seq=None):
        ...
        if att_masks is None:
            att_masks = att_feats.new_ones(att_feats.shape[:2], dtype=torch.long) # att_feats： (batch_wize, 196, 2048) --> (batch_wize, 196)
        att_masks = att_masks.unsqueeze(-2) # (batch_wize, 196) --> torch.Size([50, 1, 196])
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        # print('att_feats inside the model', att_feats.shape) # att_feats inside the model torch.Size([25, 6, 512])
        att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks, seq)

        out = self.model(att_feats, seq, att_masks, seq_mask) # tgt_mask is seq_mask
...
# https://its201.com/article/unamable/120731376
