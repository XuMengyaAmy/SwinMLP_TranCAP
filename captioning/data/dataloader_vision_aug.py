...
class DataLoader:
    def __init__(self, opt):
        ...
        print('=================================')
        print('DataLoader for vision with augmentation')
        print('=================================')
        ...
        self.dataset = Dataset(opt, transform = val_transform)
        
        # Initialize loaders and iters
        self.loaders, self.iters = {}, {}
        for split in ['train', 'val', 'test']:
            if split == 'train':
                self.train_dataset = Dataset(opt, transform = train_transform)
                sampler = MySampler(self.train_dataset.split_ix[split], shuffle=True, wrap=True) # self.dataset.split_ix[split]
                self.loaders[split] = data.DataLoader(dataset=self.train_dataset,
                                                    batch_size=self.batch_size,
                                                    sampler=sampler,
                                                    pin_memory=True,
                                                    num_workers=4, # 4 is usually enough
                                                    collate_fn=partial(self.train_dataset.collate_func, split=split),
                                                    drop_last=False) 
            else:
                self.val_dataset = Dataset(opt, transform = val_transform)
                sampler = MySampler(self.val_dataset.split_ix[split], shuffle=False, wrap=False)
                self.loaders[split] = data.DataLoader(dataset=self.val_dataset,
                                                    batch_size=self.batch_size,
                                                    sampler=sampler,
                                                    pin_memory=True,
                                                    num_workers=4, # 4 is usually enough
                                                    collate_fn=partial(self.val_dataset.collate_func, split=split),
                                                    drop_last=False) 
