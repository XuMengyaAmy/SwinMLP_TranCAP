...
def get_useful_start_idx(sequence_length, list_each_length): # sequence_length is 10, list_each_length stores #frames in these folders
    count = 0
    idx = []
    for i in range(len(list_each_length)): # len(list_each_length) = 40, 40个元素意味着40个视频，每个元素的值意味着 每个视频有多少帧图片
        for j in range(count, count + (list_each_length[i] + 1 - sequence_length)): # range(0, 0+ (1734+1-10)) = range(0, 1734-9)
            idx.append(j)
...
class Dataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        # separate out indexes for each of the provided splits
        self.split_ix = {'train': [], 'val': [], 'test': []}
        for ix in range(len(self.info['images'])): # ix是从0开始的索引
            ...
            if not 'split' in img:
                ...
                self.split_ix['train'].append(ix) # ix 刚好和 “id”是一致的
            elif img['split'] == 'val':
                self.split_ix['val'].append(ix)
            elif img['split'] == 'test':
                self.split_ix['test'].append(ix)
        ...
        self.num_each_seq_train = [137, 134, 148, 129, 149, 135, 149, 138, 146, 147, 148]
        self.num_each_seq_val = [149, 149, 149]
    ...
    def __getitem__(self, index):
        ...
        # print('id of the image', self.info['images'][ix]['id'])
        # if ix == 132:
        # print('id of the image when the index is 132', self.info['images'][132]['id']) # 132


        # 在绝对id的世界里，ix 和 self.info['images'][ix]['id'] 并非是一样的，因为已经分了 train 和 valid
        # 在分了train, valid的情况下，二者并不是一样的. 比如 “id” 是 568， 但是它在 train split中对应的 ix 索引是 149
        # 所以 ix 应该永远都是从0开始的。
...
class DataLoader:
    def __init__(self, opt):
        ...
        num_each_seq_train = self.dataset.num_each_seq_train
        num_each_seq_val = self.dataset.num_each_seq_val
        print('num_each_seq_train', num_each_seq_train) # num_each_seq_train [137, 134, 148, 129, 149, 135, 149, 138, 146, 147, 148]
        print('num_each_seq_val', num_each_seq_val) # num_each_seq_val [149, 149, 149]
        
        train_useful_start_idx = get_useful_start_idx(opt.sequence_length, num_each_seq_train)
        # print('train_useful_start_idx', train_useful_start_idx) # [0, 1, ..,  131, 132, || 137, 138, 139, ]
        ...
        # # train_useful_start_idx 存储的是那些真的会被用作为“视频片段”开始帧的id值
        # # train_useful_start_idx 的值就是 我们将会形成多少个视频片段，注意：这些视频片段的图片帧之间是有重复的
        
        val_useful_start_idx = get_useful_start_idx(opt.sequence_length, num_each_seq_val)
        ...
        for split in ['train', 'val', 'test']:
            if split == 'train':
                ...
                # # mengya; for temporal case
                sampler = MySampler(train_idx, shuffle=False, wrap=True) # shuffle must be false in train to keep the certain sequence in temporal case
                # sampler = MySampler(self.dataset.train_idx, shuffle=False, wrap=True)
                ...
                # sampler = MySampler(self.dataset.split_ix[split], shuffle=False, wrap=False)
                # mengya; for temporal case
                sampler = MySampler(val_idx, shuffle=False, wrap=False)
                # sampler = MySampler(self.dataset.val_idx, shuffle=False, wrap=False)
...
# class SeqSampler(data.sampler.Sampler):
...
'''
        ...
        for i, data in enumerate(train_loader_80):
            ...
            inputs = inputs.view(-1, sequence_length, 3, 224, 224) # 把原本的batch size按照 sequence_length分成了(-1, sequence_length)
